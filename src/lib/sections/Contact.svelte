...
<div
	...
	class="flex w-full flex-col justify-between gap-y-16 tracking-tight sm:gap-y-24 {isInView
		? 'animate'
		: 'opacity-0'}"
>
	<div class="flex flex-col gap-y-6">
		<h3 class="text-5xl font-normal md:text-6xl">{data.contact.title}</h3>
		...
		{#each data.contact.links as link, index}
			<a
				class="group flex transition-all duration-300 ease-in-out
					{index === 0 ? 'mb-2 text-3xl md:text-4xl' : ''}"
				href={link.link}
				target="_blank"
			>
				<span
